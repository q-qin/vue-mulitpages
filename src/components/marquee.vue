<template>
    <div class="broadcast-window" :style="{height: height + 'rem','z-index':zindex}">
        <ul class="broadcast-container" ref="container">
            <slot></slot>
        </ul>
    </div>
...
export default {
  ...
  methods: {
    ...
    fixList() {
      ...
      this.$nextTick(() => {
        // 用总高度/length，减小较小
        // this.height = Math.ceil(this.$refs.container.offsetHeight / (this.length+1));
        this.zindex = 0;
      });
    },
    /*
    ...
    start() {
      ...
      setInterval(() => {
        ...
        if (this.direction === "up") {
          currenTranslateY = Number(-this.currentIndex * this.height).toFixed(2) + "rem";
        } else {
           currenTranslateY = Number(-(this.currentIndex + 1) * this.height).toFixed(2) + "rem";
        }
        this.setTransform(
          this.$refs.container,
    ...
    quickJump(toFirst) {
      ...
      if (toFirst) {
        // 跳转到首个item
        this.currentIndex = 0;
        currenTranslateY = "0rem";
      } else {
        this.currentIndex = this.length - 1;
        currenTranslateY = -(this.currentIndex + 1) * this.height + "rem";
      }
